...
class Net(torch.nn.Module):
    ...
    def __init__(self, args, params, class_num):
        ...
        #################################################################################
        # 替换为transformer层, 768dim n-head=14, 300dim n-head=10
        self.trans_encoder = nn.TransformerEncoderLayer(d_model=feature_dim, nhead=10, dropout=0.1, dim_feedforward=feature_dim*4, activation='gelu', batch_first=True)
        # 测试后发现3层效果最好
        self.trans_layer = nn.TransformerEncoder(self.trans_encoder, num_layers=3)
        self.trans_norm = nn.LayerNorm(feature_dim)
