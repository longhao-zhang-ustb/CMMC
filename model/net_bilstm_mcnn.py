...
class Net(torch.nn.Module):
    ...
    def __init__(self, args, params):
        ...
        ##############################BiLSTM##############################
        self.origin_dense = nn.Linear(feature_dim, 128)
        self.bilstm_layer = nn.LSTM(input_size=128, hidden_size=self.bilstm_gru_hidden_dim, num_layers=4, batch_first=True, bidirectional=True)
        self.bilstm_dropout = nn.Dropout(0.3)
        self.convs = nn.ModuleList([nn.Conv1d(self.bilstm_gru_hidden_dim*2, params.kernel_num, kernel_size=kernel_size) \
        ...
        self.convs_pool = nn.AdaptiveAvgPool1d(256)
    ...
    def forward(self, words, pos1, pos2):
        ...
        ##############################BiLSTM-MCNN#################################
        lstm_in = self.origin_dense(features)
        lstm_out,_ = self.bilstm_layer(lstm_in)
        x = self.bilstm_dropout(lstm_out)
